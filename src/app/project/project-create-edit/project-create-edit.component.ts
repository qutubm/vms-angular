...
export class ProjectCreateEditComponent implements OnInit {
  ...
  projectForm : FormGroup = this.formBuilder.group({
    ...
    estimatedCompletionDate: ['', [ Validators.minLength(2), Validators.maxLength(50)]],
    actualCompletionDate:['', [Validators.maxLength(50)]],
  ...
  estimatedTaskCompletionDate : FormControl = new FormControl('', [Validators.required]);
  ...
  constructor(private formBuilder: FormBuilder, private projectServices: ProjectService, private activatedRoute: ActivatedRoute, private router: Router, private modalService : NgbModal) {
  }
  ...
  ngOnInit(): void {
    ...
    this.modalPopup.subscribe(res => {
        ...
        if(res == 'closeTask')
        {
            ...
            {
              if(this.taskId.value == null || this.taskId.value == undefined || this.taskId.value == '')
              {
  ...
  openTaskDialog()
  {
      ...
      this.resource.markAsUntouched();
      this.resource.markAsPristine();

      this.estimatedTaskCompletionDate.setValue('');
      this.estimatedTaskCompletionDate.markAsUntouched();
      this.estimatedTaskCompletionDate.markAsPristine();

      this.actualTaskCompletionDate.setValue('');
      this.actualTaskCompletionDate.markAsUntouched();
      this.actualTaskCompletionDate.markAsPristine();

      this.taskCreatedDate.setValue('');

      this.taskCompleted.setValue('');
      this.taskCompleted.markAsUntouched();
      this.taskCompleted.markAsPristine();
  ...
  editTaskDetails(taskId : string)
  {
    ...
    this.projectServices.fetchTask(taskId).subscribe(data => {
      ...
      {
          this.projectTaskForm.get('taskId').setValue(data.Task.Id);
          // this.projectTaskForm.controls.taskName.setValue(data.Task.Name);
          this.projectTaskForm.get('taskName').setValue(data.Task.Name);
          this.projectTaskForm.get('resource').setValue(data.Task.ProfileId);

          this.projectTaskForm.get('estimatedTaskCompletionDate').setValue(new NgbDate(Number(data.Task.EstimatedCompletionDate.substring(0,4)), 
          Number(data.Task.EstimatedCompletionDate.substring(4,6)), 
          Number(data.Task.EstimatedCompletionDate.substring(6,8))));

          this.projectTaskForm.get('actualTaskCompletionDate').setValue(new NgbDate(Number(data.Task.ActualCompletionDate.substring(0,4)), 
                Number(data.Task.ActualCompletionDate.substring(4,6)), 
                Number(data.Task.ActualCompletionDate.substring(6,8))));

          this.projectTaskForm.get('taskCreatedDate').setValue(data.Task.CreatedDate);
          this.projectTaskForm.get('taskCompleted').setValue(data.Task.Completed);
  ...
  saveTask()
  {
    ...
    var task = new Task();
    task.Id = this.taskId.value; 
    task.Name = this.taskName.value;
    task.ProfileId = this.resource.value;
    task.ProjectId = this.projectId;

    if(this.actualTaskCompletionDate.value !== null || this.actualTaskCompletionDate.value !== undefined)
    {
        let actualTaskCompletion = this.actualTaskCompletionDate.value as NgbDate;
    ...
    let estimatedTaskCompletion = this.estimatedTaskCompletionDate.value as NgbDate;
    ...
    {
      task.CreatedDate =  this.taskCreatedDate.value;
    ...
    if(this.taskCompleted.value === 'true' )
    {
        task.Completed = 'Y';
    }
    else
